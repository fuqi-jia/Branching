...
from omt_branching.solver.vsids_trace import VSIDSTraceConfig, build_vsids_examples_sat
...
def main() -> None:
    ...
    ap.add_argument("--teacher", choices=["vsids", "lookahead"], default="vsids",
                    help="imitation 冷启动教师:vsids 轨迹(默认)或旧 look-ahead")
    ap.add_argument("--max-ex", type=int, default=40, help="vsids 教师:单实例样本上限(0=不限)")
    args = ap.parse_args()
    torch.manual_seed(0)
    ...
    if args.train > 0:
        ...
        if args.teacher == "vsids":
            exs = [e for e in build_vsids_examples_sat(
                       tr_probs, VSIDSTraceConfig(max_examples=args.max_ex))
        ...
        print(f"imitation({args.teacher}): {len(exs)} 样本, "
              f"branch {h[0].get('branch', 0):.3f}->{h[-1].get('branch', 0):.3f}")
    if args.rl_iters > 0:
